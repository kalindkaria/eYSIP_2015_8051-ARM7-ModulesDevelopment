...
* File name: 360_degree_rotation.c
* Functions: delay_ms(unsigned int), right(), stop()
...
*        take 360 degree turn in clockwise direction.
...
int main()
{
	Init_motion_pin();	//initialize motor pins
  	right();			//turn right
	delay_ms(3500);		//delay of 3.5 seconds
	stop();				//stop
	while(1);
}//main ends here
