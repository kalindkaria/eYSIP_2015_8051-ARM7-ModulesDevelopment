...
* File name: lcd_string_display.c
...
void main()
{
	...
	lcd_cursor(1,5);	//places cursor at 5th column of 1st row
	lcd_string("e-yantra");	//writes string on LCD screen
	lcd_cursor(2,4);		//places cursor at 4th column of 2nd row
	lcd_string("Firebird V");  //writes string on LCD screen
	while(1);
