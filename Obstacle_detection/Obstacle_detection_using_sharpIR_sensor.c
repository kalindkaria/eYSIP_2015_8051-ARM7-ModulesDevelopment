...
* File name: obstacle_avoiding_using_sharpIR_sensor.c
* Functions: ADC_conversion(unsigned char ADC_channel_number), forward(), right(), stop(), 
...
* Description: This experiment demonstrates use of sharpIR sensor in avoiding an obstacle.
...
void main()
{
	unsigned int sharpIR_value=0; //variable to store sharp IR sensor value
 	...
 	forward();
 	left_motor_velocity(0x2F);  /*0x00 will give full (100% duty cycle) velocity, 
while 0xFF will give zero (0% duty cycle) velocity. Any value in between 0x00 and 0xFF will give intermediate velocity.*/			
	right_motor_velocity(0x18); /*0x00 will give full (100% duty cycle) velocity, 
while 0xFF will give zero (0% duty cycle) velocity. Any value in between 0x00 and 0xFF will give intermediate velocity.*/
 while(1)
 {
	sharpIR_value=ADC_conversion(2);
