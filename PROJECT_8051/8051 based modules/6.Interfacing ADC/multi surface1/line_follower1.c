...
*                  unsigned char flag
* This experiment demonstrates multisurface line following on both white as well as black by monitoring the white line sensor's values.
...
* Logic: constantly monitors the white line sensors value and checks all conditions to accordingly adjusts its path.
* Example Call: Called automatically by the Operating System
*
*/
void main()
{
	unsigned int left_sensor_value=0;  	//variable to store left sensor value
	unsigned int middle_sensor_value=0;	//variable to store middle sensor value
	unsigned int right_sensor_value=0;	//variable to store right sensor value
 ...
 while(1)
 {
	...
	if((left_sensor_value>50 && middle_sensor_value<20 && right_sensor_value>50 && flag==0) ||(left_sensor_value<20 && middle_sensor_value>50 && right_sensor_value<20 && flag==0)) //checks if middle sensor is on white line and rest two on black or viceversa
	{
	...
	if(left_sensor_value<50 && right_sensor_value>50 && flag==0 && middle_sensor_value<20) //if it deviates towards right and on white line
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		left();	//soft left turn by varying velocity
		while(left_sensor_value>50);
	}
	if(left_sensor_value>50 && right_sensor_value<50 && flag==0 && middle_sensor_value<20) //if it deviates towards left and on white line
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		right(); //soft right turn by varying velocity
		while(right_sensor_value>50);
	}
	if(left_sensor_value>20 && right_sensor_value<20 && flag==0 && middle_sensor_value>50) //if it deviates towards right and on black line
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		left();	//soft left turn by varying velocity
		while(left_sensor_value<20);
	}
	if(left_sensor_value<20 && right_sensor_value>20 && flag==0 && middle_sensor_value>50) //if it deviates towards left and on black line
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		right(); //soft right turn by varying velocity
		while(right_sensor_value<20);
	}
	if((left_sensor_value>50 && middle_sensor_value>50 && right_sensor_value>50 && flag==0)||(left_sensor_value<20 && middle_sensor_value<20 && right_sensor_value<20 && flag==0)) //if all sensors are on black or on white
		stop();	 //stops
	}
}//main ends here