...
* File name: wall_follower.c
* Functions: ADC_conversion(unsigned char ADC_channel_number), forward(), right(), left(), stop(), 
...
* Description: This experiment demonstrates use of proximity sensor to remain in proximity of a wall.
...
* Logic: constantly monitors the sensor value and checks for conditions and acordingly adjusts its path .  
...
void main()
{
 ...
 while(1)
 {
	...
	if(sensor2_value>150) //checks for right deviation
	{
		left();
	}
	else if(sensor2_value<148) //checks for left deviation
	{
		right();
	}	
	else
	...
	if(sensor3_value<40) //checks for an obstacle
	stop();	
 }
} //main ends here