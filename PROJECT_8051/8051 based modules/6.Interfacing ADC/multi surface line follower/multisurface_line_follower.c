...
RB=0;
LB=0;
left_motor_velocity(0x4F);	//increase left motor velocity 
 right_motor_velocity(0x4F);  //decrease  right motor velocity
}
/* Function name - left
* Input - none 
...
{
	if(left_sensor_value<20 && middle_sensor_value>50 && right_sensor_value<20 && flag==0) //checks if middle sensor is on white line and rest two on black
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		forward();  //left and right motor is at same velocity
	}
	if(left_sensor_value>20 && right_sensor_value<20 && flag==0) //if it deviates towards right
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		left();	//soft left turn by varying velocity
		while(left_sensor_value<20);
	}
	if(left_sensor_value<20 && right_sensor_value>20 && flag==0) //if it deviates towards left
	{
		flag=1;	//set the flag to 1 so that further white line sensor comparision is disabled
		right(); //soft right turn by varying velocity
		while(right_sensor_value<20);
	}
	if(left_sensor_value<20 && middle_sensor_value<20 && right_sensor_value<20 && flag==0) //if all sensors are on black
		stop();	 //stops
}
/*
* Function Name: main
...
{  	
	...
	if(left_sensor_value<20 && right_sensor_value<20)
	{
	while(left_sensor_value<20 && right_sensor_value<20)
		{
		lcd_cursor(2,1);
		lcd_string("black line");
		black_line_follow();
		}
	}
	else
	{
	while(left_sensor_value>50 && right_sensor_value>50)
		{
		lcd_cursor(2,1);
		lcd_string("white line");
		white_line_follow();
		}
	}
}
